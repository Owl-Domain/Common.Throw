...
public static partial class ThrowIfArgumentExtensions
{
   ...
   ///   was higher than the (inclusive) <paramref name="threshold"/> value.
   /// </summary>
   ...
   ///   Thrown if the given <paramref name="argument"/> was higher than the (inclusive) <paramref name="threshold"/> value.
   /// </exception>
   [StackTraceHidden]
   public static IThrowIfArgument IsTooHigh<T>(
      ...
      [CallerArgumentExpression(nameof(argument))] string argumentExpression = "<argument>")
   {
      if (argument.CompareTo(threshold) > 0)
         Throw.For.ArgumentOutOfRange(argumentExpression, argument, $"The given argument value was higher than the allowed threshold of ({threshold}).");
